...
| **F2** | `tensor` lib + GEMM gadget (AIR) with positive+negative tests — **backend done** (`libs/stark/`: FFT over the norm-1 torus, constraint IR, column commitment, RLC composition, quotient, FRI, query-time verification) **plus** operand binding to the quantized weights (nibble range + dequant equation), 16-MAC chunking with its own binding, a compiled routing AIR proving the selected set IS the top-k, a LogUp core, and a bit-exact fp16 multiply (108 composed constraints, measured); **still pending**: the real-engine witness and the per-format weight layer | backend negatives green (tampered trace/opening/quotient/root, random trace); every rounding witness of the fp16 AIR is flipped and rejected |
