...
| **F2** | `tensor` lib + GEMM gadget (AIR) with positive+negative tests — **backend done** (`libs/stark/`: FFT over the norm-1 torus, constraint IR, column commitment, RLC composition, quotient, FRI, query-time verification) **plus** operand binding to the quantized weights (nibble range + dequant equation + an fp16 scale-provenance gadget, `u = ±(2^10+m)·2^s`), transition-exempt closing rows (divisor `Z_H/Π`, so the synthetic row carries no operand binding), 16-MAC chunking with its own binding (one shared scale per chunk, since 16 divides every Q4_K block size), a compiled routing AIR proving the selected set IS the top-k, a LogUp core, a format-generic bit-exact float multiply covering zero, infinity and NaN as inputs, subnormal RESULTS and the underflow to zero, with a single rounding across the whole range (258/394/158/170 composed constraints for binary16/bf16/fp8-e4m3/fp8-e5m2, measured), exact widening to fp32 (48–62 constraints, no rounding to get wrong) and a tested barrel-shifter gadget; **still pending**: the real-engine witness, the per-format weight layer and fp32 accumulation | backend negatives green (tampered trace/opening/quotient/root, random trace); every rounding witness of every format's AIR is flipped and rejected, and ~1465 pairs per format are swept against the reference with every constraint evaluated; all 65536 bfloat16 patterns are widened exhaustively |
...
- [docs/soundness-operand-binding.md](docs/soundness-operand-binding.md) — design notes on F2 soundness, with the measurements behind the choices (Spanish)
